<?php
    ...
    $sql = "SELECT m.id AS member_id, u.name, u.email FROM members m JOIN users u ON m.user_id = u.id WHERE u.id = ?";
    ...
    $sql = "SELECT plan_details, start_date, end_date FROM workout_plans WHERE member_id = ?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$member_id]);
    $workouts = $stmt->fetchAll();

    $sql = "SELECT membership_type, start_date, end_date FROM memberships WHERE member_id = ?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$member_id]);
    $membership = $stmt->fetch();

    $sql = "SELECT date, status FROM attendance WHERE member_id = ? ORDER BY date DESC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$member_id]);
