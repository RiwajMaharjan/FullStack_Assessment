...
<body>
    ...
    <main>
        <?php if (isset($_SESSION['error'])): ?>
            <div class="alert alert-error"
                style="background: #fee2e2; color: #991b1b; padding: 1rem; border-radius: 8px; margin-bottom: 1.5rem; border: 1px solid #fecaca;">
        ...
        <section>
                    ...
                    <?php foreach ($members as $index => $m): ?>
                        <tr class="<?= $index >= 5 ? 'hidden-item-toggle' : '' ?>">
                            ...
                            <td style="display: flex; gap: 0.5rem;">
                                <button class="btn action-edit edit-member-btn" data-id="<?= $m['id'] ?>"
                                    data-name="<?= htmlspecialchars($m['name']) ?>"
                                    data-email="<?= htmlspecialchars($m['email']) ?>"
                                    data-phone="<?= htmlspecialchars($m['phone']) ?>"
                                    data-membership_id="<?= $m['membership_id'] ?>" data-join_date="<?= $m['join_date'] ?>"
            ...
            <?php if (count($members) > 5): ?>
                <button class="view-more-btn" data-target="membersTable">View More <i
        ...
        <section>
                ...
                <tbody id="membershipsTable">
                    <?php foreach ($memberships as $index => $ms): ?>
                        <tr class="<?= $index >= 5 ? 'hidden-item-toggle' : '' ?>">
                            ...
                            <td><?= $ms['price'] ?></td>
                            <td style="display: flex; gap: 0.5rem;">
                                <button class="btn action-edit edit-membership-btn" data-id="<?= $ms['id'] ?>"
                                    data-name="<?= htmlspecialchars($ms['name']) ?>"
                                    data-duration_months="<?= $ms['duration_months'] ?>" data-price="<?= $ms['price'] ?>">
                                    <i class="fas fa-edit"></i> Edit
                                </button>
                                ...
                                <form action="../handlers/delete_membership.php" method="POST" style="margin: 0;">
                                    ...
                                    <input type="hidden" name="csrf_token" value="<?= generateCSRFToken(); ?>">
                                    <button class="btn action-delete" onclick="return confirm('Delete this membership?')">
                                        <i class="fas fa-trash"></i> Delete
                                    </button>
                                </form>
                            </td>
            ...
            <?php if (count($memberships) > 5): ?>
                <button class="view-more-btn" data-target="membershipsTable">View More <i
                        class="fas fa-chevron-down"></i></button>
            <?php endif; ?>
        </section>
        ...
        <section>
                ...
                <tbody id="workoutsTable">
                    <?php foreach ($workout_plans as $index => $w): ?>
                        <tr class="<?= $index >= 5 ? 'hidden-item-toggle' : '' ?>">
                            ...
                            <td><?= $w['end_date'] ?></td>
                            <td style="display: flex; gap: 0.5rem;">
                                <button class="btn action-edit edit-workout-btn" data-id="<?= $w['id'] ?>"
                                    data-member_id="<?= $w['member_id'] ?>"
                                    data-plan_details="<?= htmlspecialchars($w['plan_details']) ?>"
                                    data-start_date="<?= $w['start_date'] ?>" data-end_date="<?= $w['end_date'] ?>">
                                    <i class="fas fa-edit"></i> Edit
                                </button>
                                ...
                                <form action="../handlers/delete_workout.php" method="POST" style="margin: 0;">
                                    ...
                                    <input type="hidden" name="csrf_token" value="<?= generateCSRFToken(); ?>">
                                    <button class="btn action-delete" onclick="return confirm('Delete this workout?')">
                                        <i class="fas fa-trash"></i> Delete
                                    </button>
                                </form>
                            </td>
            ...
            <?php if (count($workout_plans) > 5): ?>
                <button class="view-more-btn" data-target="workoutsTable">View More <i
                        class="fas fa-chevron-down"></i></button>
            <?php endif; ?>
        </section>
        ...
        <section>
                ...
                <tbody id="attendanceTable">
                    <?php foreach ($attendance as $index => $a): ?>
                        <tr class="<?= $index >= 5 ? 'hidden-item-toggle' : '' ?>">
                            ...
                            <td><?= $a['status'] ?></td>
                            <td style="display: flex; gap: 0.5rem;">
                                <button class="btn action-edit edit-attendance-btn" data-id="<?= $a['id'] ?>"
                                    data-member_id="<?= $a['member_id'] ?>" data-date="<?= $a['date'] ?>"
                                ...
                                <form action="../handlers/delete_attendance.php" method="POST" style="margin: 0;">
                                    ...
                                    <input type="hidden" name="csrf_token" value="<?= generateCSRFToken(); ?>">
                                    <button class="btn action-delete" onclick="return confirm('Delete this attendance?')">
                                        <i class="fas fa-trash"></i> Delete
                                    </button>
                                </form>
                            </td>
            ...
            <?php if (count($attendance) > 5): ?>
                <button class="view-more-btn" data-target="attendanceTable">View More <i
                        class="fas fa-chevron-down"></i></button>
            <?php endif; ?>
        </section>
    ...
    <script src="../../assets/js/members_search.js?v=<?= time() ?>"></script>
